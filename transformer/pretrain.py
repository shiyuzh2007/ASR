...
def train(config, num_epoch, last_pretrain_model_dir, pretrain_model_dir, model_dir, block_idx):
    logger = logging.getLogger('')
    ...
    #         block_idx - 1)
    logger.info(
        "config.num_blocks=" + str(config.num_blocks) + ',config.train.var_filter=' + str(config.train.var_filter))
